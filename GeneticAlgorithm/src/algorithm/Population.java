...
public class Population {
	...
	*/
	...
	public void crossOver(){
		...
		for(int i = 0; i < n; i++){
			...
			/*if(parent1.fitness < parent1.n * 0.5){
				...
				tempPopulation[i] = normalBirth(parent1, parent2);
			//}
	...
	public void replaceTwin(){
		int count = 0;
		int twin = 0;
		for(int i = 1; i < this.n; i++){
			for(int k = 0; k < i; k++){
				count = 0;
				for(int j = 0; j < population[j].n; j++){
					if(population[i].queens[j] != population[k].queens[j]){
						count++;
					}
				}
				if(count<=population[i].n*0.02){
					twin++;
					System.out.println("Old: " + population[k]);
					System.out.println("Twin: " + population[i]);
		...
		System.out.println("Total twin: " + twin);
	}
	...
	public void mutation(){
		...
		for(int i = 0; i < n; i++){
			...
			population[i].errors = getError(population[i]);
			/*
